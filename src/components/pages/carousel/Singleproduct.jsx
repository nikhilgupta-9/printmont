...
const SingleProduct = ({
  ...
  // Fetch from API if apiUrl is provided
  useEffect(() => {
    if (!apiUrl) {
      // Map static/local products to make sure they have formatted fields
      const formatted = initialProducts.map(p => ({
        id: p.id || '',
        title: p.title || p.name || '',
        img: p.img || p.image || '/default-img.jpg',
        price: p.price || '',
  ...
  return (
    <div
      className={`horizontal-scroll-wrapper position-relative mx-0 py-2 border bd ${backgroundImageUrl ? 'custom-bg-image' : ''}`}
      style={{
      ...
      >
        {products.map((product, index) => (
          <Link
            ...
            className="scroll-card border d-flex flex-column p-1 h-100 m-0 text-decoration-none text-dark bg-white"
            style={getResponsiveCardStyle(screenSize)}
          >
            ...
            <div className="card-body text-center p-1 w-100 flex-grow-1 d-flex flex-column justify-content-between">
              <p
                className="product-name text-truncate d-block fs-6 mt-1 text-center mb-1"
                style={{ maxWidth: "100%", textDecoration: 'none' }}
              >
                {product.title}
              </p>
              <p className="mb-0 p-0 small text-center text-dark">
                {product.price && <span className="fw-bold fs-6">₹{product.price}</span>}{" "}
                {product.originalPrice && (
                  <del className="text-muted ms-1" style={{ fontSize: "0.7rem" }}>
                    ₹{product.originalPrice}
                  </del>
                )}
                {" "}
                {product.discount && <span className="text-success fw-bold small ms-1">{product.discount}</span>}
              </p>
            </div>
          </Link>
        ))}
