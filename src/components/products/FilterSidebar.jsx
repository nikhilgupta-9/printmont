...
const FilterSidebar = ({
  ...
  return (
    <div className="ap-filter-sidebar bg-white p-3 h-100 d-flex flex-column">
      {/* Mobile Top Header */}
      ...
      <div className="d-none d-md-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
        <span className="fw-bold fs-5 text-dark">Filters</span>
        <Button
          variant="link"
          size="sm"
          onClick={onClearAll}
          className="text-primary text-decoration-none p-0 fw-bold text-uppercase ap-clear-btn"
        >
          Clear All
        </Button>
      </div>

      <div className="ap-filter-scroll-container flex-grow-1">
        {/* CATEGORIES SECTION */}
        {facets.categories && facets.categories.length > 0 && (
          <div className="ap-filter-section mb-3 border-bottom pb-3">
            <h6 className="ap-filter-heading text-uppercase text-dark fw-bold mb-2">Categories</h6>
        ...
        <Accordion defaultActiveKey={["brand", "price", "discount"]} alwaysOpen flush className="ap-filter-accordion">
          ...
          {facets.brands && facets.brands.length > 0 && (
            <Accordion.Item eventKey="brand" className="border-bottom">
              <Accordion.Header className="py-1">
                <span className="fw-bold text-uppercase ap-accordion-title">Brand</span>
              </Accordion.Header>
              <Accordion.Body className="pt-1 pb-3 px-1">
                {facets.brands.length > 6 && (
                  <InputGroup size="sm" className="mb-2">
                    ...
                    <Form.Control
                      ...
                      className="border-start-0 bg-light ap-filter-search-input"
                    />
                  </InputGroup>
                )}
                ...
                <div className="ap-checkbox-group">
                  {visibleBrands.map((b) => (
                    <Form.Check
                      ...
                      id={`brand-${b.name}`}
                      label={
                        <span className="d-flex justify-content-between w-100 ap-filter-label">
                          <span>{b.name}</span>
                          <span className="ap-count ms-1">({b.count})</span>
                        </span>
                      }
                  ...
                  {filteredBrands.length === 0 && (
                    <small className="text-muted d-block py-1 fs-7">No brands match search</small>
                  )}
                </div>
                ...
                {filteredBrands.length > 7 && (
                  <button
                    ...
                    className="btn btn-link p-0 text-primary text-decoration-none ap-clear-btn fw-semibold mt-1"
                  >
          ...
          <Accordion.Item eventKey="price" className="border-bottom">
            <Accordion.Header className="py-1">
              <span className="fw-bold text-uppercase ap-accordion-title">Price</span>
            </Accordion.Header>
            <Accordion.Body className="pt-2 pb-3 px-2">
              ...
              <div className="d-flex justify-content-between align-items-center mt-2 ap-price-badge fw-semibold">
          ...
          <Accordion.Item eventKey="discount" className="border-bottom">
            <Accordion.Header className="py-1">
              <span className="fw-bold text-uppercase ap-accordion-title">Discount</span>
            </Accordion.Header>
            <Accordion.Body className="pt-1 pb-3 px-1">
              {discountOptions.map((opt) => (
                <Form.Check
                  ...
                  label={<span className="ap-filter-label">{opt.label}</span>}
          ...
          {facets.sizes && facets.sizes.length > 0 && (
            <Accordion.Item eventKey="size" className="border-bottom">
              <Accordion.Header className="py-1">
                <span className="fw-bold text-uppercase ap-accordion-title">Size</span>
              </Accordion.Header>
              <Accordion.Body className="pt-1 pb-3 px-1">
          ...
          <Accordion.Item eventKey="availability" className="border-0">
            <Accordion.Header className="py-1">
              <span className="fw-bold text-uppercase ap-accordion-title">Availability</span>
            </Accordion.Header>
            <Accordion.Body className="pt-1 pb-2 px-1">
              <Form.Check
                type="checkbox"
                id="stock-filter"
                label={<span className="ap-filter-label">Exclude Out of Stock</span>}
