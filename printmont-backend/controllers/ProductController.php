    ...
    public function addProduct($data, $files)
{
    ...
    public function getDiscountProductsApi()
    {
        try {
            ...
            $formatted = [];
            foreach ($products as $product) {
                $formatted[] = $this->formatProductForApi($product);
            }
    ...
    public function getGroupedCategoriesProductsApi()
    {
        try {
            $categories = $this->categoryModel->getParentCategories();
            $grouped = [];
            foreach ($categories as $cat) {
                $products = $this->productModel->getProductsByCategory($cat['id']);
                ...
                foreach ($products as $p) {
                    ...
                    $imgUrl = '';
                    if (!empty($formatted['images']) && is_array($formatted['images'])) {
                        $imgUrl = $formatted['images'][0]['image_url'] ?? '';
                    }
                    $items[] = [
                        'title' => $formatted['name'] ?? '',
                        'discount' => !empty($formatted['discount_price']) ? 'Special Offer' : '',
                        'image' => $imgUrl ?: '/default-img.jpg'
                    ];
                    $count++;
                }
                ...
                if (!empty($items)) {
                    $grouped[] = [
                        'title' => $cat['name'] . ' Items',
                        'items' => $items
                    ];
                }
            }
            return ['success' => true, 'data' => array_slice($grouped, 0, 3)];
        } catch (Exception $e) {
            return ['success' => false, 'error' => $e->getMessage()];
        }
    }
